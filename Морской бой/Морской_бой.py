...
class BoardOutException(BoardException):
    def __str__(self):
        return 'Выводить за пределы поля ЗАПРЕЩЕНО!'

class BoardUsedException(BoardException):
    def __str__(self):
        return 'В данную область уже был произведен выстрел!'
            ...
            for ship in self.ships:
                if ship.is_hit(d):
                    ...
                    if ship.lives == 0:
                        ...
                        self.contour(ship, cisible=True)
                        print('Цель уничтожена!')
                        self.last_hit = []
                        return False
                else:
                    print('Цель повреждена!')
                    self.last_hit.append(d)
                    return True
            self.field[d.x][d.y] = '.'
            print('Miss') #Мемы с TF2 XD
            return False
    ...
    def ask(self) -> Dot:
        while True:
            ...
            if not all((x.isdigit(), y.isdigit())):
                print('Координаты определяются числами')
                continue
